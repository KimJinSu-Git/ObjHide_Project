...
- [x] **역할별 시점 시스템 구현** (술래: 1인칭 FPS + 총기 모델 / 도망자: 3인칭 TPS)
- [x] **자유 시점(FreeLook) 전환 및 고정 시스템** (도망자 전용, L키 또는 버튼 토글)
- [x] **사망 시 관전 시스템 기초** (사망 시 자유 시점으로 전환하여 맵 탐방 가능)
...
- [ ] **PropPlayer 레이어 분리**: `OnTriggerEnter` 시 레이어 기반 타격 판정 정교화.

### Phase 2: 핵심 게임 루프 완성
- [x] 라운드 승패 판정 로직 구현
- [x] 결과창 UI 연동 (승리팀 표시 및 씬 재시작 로직)
- [ ] 게임 재시작 및 로비 복귀 흐름 정교화
- [ ] UI 폴리싱: 화면 중앙 조준점(Crosshair) 및 시점 전환 전용 버튼 UI 추가.

### Phase 3: 폴리싱 및 콘텐츠 확장
- [ ] 사물 데이터베이스(`PropDatabase`) 확장 (복합 콜라이더 데이터 포함 고려)
